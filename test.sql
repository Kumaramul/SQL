...
END //
DELIMITER ;
-- CALL get_average_days('9', @A);
-- select @A;
-- select * from  borrowed_books  where book_id=8;


-- answer 3

SELECT u.* FROM borrowed_books bb LEFT JOIN users u on u.user_id=bb.user_id GROUP BY  bb.user_id ORDER BY COUNT(*) DESC LIMIT 1;
...
SET @author_name = 'HHH';  
SELECT u.* FROM borrowed_books bb  RIGHT JOIN users u on u.user_id=bb.user_id LEFT JOIN  books bk on bk.book_id=bb.book_id  where bk.author=@author_name;
